...
/**
 * Résout le pack croisé d'un produit (voir mergeShopifyProduct) en récupérant le produit et la
 * variante réellement bundlés (ex : une carte SD vendue comme produit Shopify séparé), avec leur
 * vrai prix Shopify. Renvoie null si le pack n'est pas activé ou si le produit lié est introuvable
 * (supprimé/dépublié depuis Shopify) — la fiche produit affiche alors juste la caméra seule.
 */
export async function getBundledVariant(pack) {
  ...
  const variant = pack.bundledVariantId
    ? bundledProduct.variants?.find((v) => v.id === pack.bundledVariantId)
    : null;
  ...
  return variant
    ? {
        ...
        name: variant.title && variant.title !== "Default Title" ? `${bundledProduct.name} — ${variant.title}` : bundledProduct.name,
        price: variant.price,
    ...
    : {
        productId: bundledProduct.id,
        variantId: bundledProduct.shopifyVariantId,
