...
export function CartProvider({ children }) {
  ...
  const api = useMemo(() => {
    function addItem({ productId, name, price, image, variant }, qty = 1) {
      setItems((prev) => {
        ...
        return [...prev, { key, productId, name, price, image, variant, qty }];
      });
    }
  ...
  return (
    <CartContext.Provider value={{ items, count, total, ...api }}>{children}</CartContext.Provider>
  );
}
