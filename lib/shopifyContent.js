...
export const emptyShopifyContent = {
  ...
  // Complément éditorial par variante Shopify (ex : "Caméra seule" / "Pack Prêt à filmer"),
  // indexé par ID de variante — voir components/admin/ShopifyContentForm.js et ProductPackSelector.
  variantInfo: {},
};
...
export function mergeShopifyProduct(shopifyProduct, content) {
  ...
  const variantInfo = c.variantInfo || {};
  const variants = variantNodes.map((v) => {
    const info = variantInfo[shopifyGidToDocId(v.id)] || {};
    return {
      id: v.id,
      title: v.title,
