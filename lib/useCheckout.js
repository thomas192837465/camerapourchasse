...
import { getVisitorId, logCheckoutStart } from "./cartActivity";

/** Redirige directement vers le paiement adapté : Shopify si panier 100% Shopify, sinon le formulaire du site. */
export function useCheckout() {
  const router = useRouter();
  const { items, total, isShopifyCart, hasMixedSources, user } = useCart();
  ...
  async function goToCheckout() {
    ...
    logCheckoutStart({ visitorId: getVisitorId(), user, items, value: total });
